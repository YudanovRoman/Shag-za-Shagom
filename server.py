...
def Create_route():
    ...
    if not account_id or account_id == '-1':
        ...
        temp = 'Profile'
    return render_template('create_route.html', title='Information', autorization=temp, image="static/img/example.png",
                           num=0, textes=[])
...
def create_route():
    ...
    name = request.form["name-route"]
    address_ll = get_coords(address)
    description = request.form["route"]
    date_time = request.form["date-time"]
    data = error_check(name, address, description, date_time)
    print(data)
    if data:
        res = make_response(render_template('create_route.html', title='Welcome', autorization=temp,
                                            image=f"static/img/error.png",
                                            num=len(data), textes=data))

        return res
    date, time = date_time.split("T")
    date = datetime.strptime(date, "%Y-%m-%d").date()
    try:
        temp_route = Route(route_type=description.split(","), ll=address_ll, weekday=date.weekday(), time=time, name=name, creator_id=account_id)
    except:
        res = make_response(render_template('create_route.html', title='Welcome', autorization=temp,
                                            image=f"static/img/error.png",
                                            num=1, textes=[["Извините! У нас неизвестная нам ошибка которую мы постараемся решить как можно быстрее"]]))
        return res
    textes = []
    for i in range(len(temp_route.get_names_org())):
    ...
    cur = con.cursor()
    id_sql = cur.execute("""SELECT MAX(id) FROM routes""").fetchone()
    con.close()
    res = make_response(render_template('create_route.html', title='Welcome', autorization=temp,
                                        image=f"static/routes_images/route_{id_sql[0]}.png",
                                        num=len(temp_route.get_names_org()), textes=textes))

    return res

def error_check(name, ll, type, dt):
    errors = []
    if name == "":
        errors.append(["ОШИБКА! Введите название маршрута"])
    if ll == "":
        errors.append(["ОШИБКА! Введите где будет проходить прогулка"])
    if type == "":
        errors.append(["ОШИБКА! Введите чтобы вы хотели бы посетить через запятую"])
    if dt == "":
        errors.append(["ОШИБКА! Введите во сколько вы будете проводить прогулку"])
    return errors
